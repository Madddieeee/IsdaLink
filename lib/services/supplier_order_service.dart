...
class SupplierOrderService {
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> ordersStream(String supplierId) {
  ...
  void createNotificationInTransaction({
    ...
    final acceptedQuantity = fulfilledQuantity ?? requestedQuantity;
  ...
  Future<void> updateOrderStatus({
    ...
    await FirebaseFirestore.instance.runTransaction((transaction) async {
      ...
      if (!isAllowedTransition(
        ...
        throw StateError('This order has already moved to another stage.');
      }
      ...
      if (newStatus.toLowerCase() == 'cancelled') {
        ...
        transaction.update(orderReference, {
          ...
          'stockRestorePending':
              orderData['stockDeducted'] == true && !restorationProcessed,
        ...
        final delivered = normalizedNewStatus == 'delivered';
        ...
        if (normalizedNewStatus == 'accepted') {
          ...
          if (requestedQuantity <= 0) {
            throw StateError('This order has an invalid requested quantity.');
          }
          ...
          final unfulfilledQuantity = requestedQuantity - acceptedQuantity;
          final partialFulfillment = unfulfilledQuantity > 0;
          ...
          if (partialFulfillment) {
            await restoreUnfulfilledQuantity(
              ...
              orderData: orderData,
              orderId: documentId,
              quantityToRestore: unfulfilledQuantity,
            );
          }

          final unitPrice = OrderHelpers.getDoubleValue(orderData, 'unitPrice');
          ...
          transaction.update(orderReference, {
            ...
            'fulfilledTotalAmount': unitPrice * acceptedQuantity,
            'reservedQuantity': acceptedQuantity,
            'partialFulfillment': partialFulfillment,
            'fulfillmentStatus': partialFulfillment ? 'partial' : 'full',
            'partialStockRestored': partialFulfillment,
            'partialRestoredQuantity': unfulfilledQuantity,
            if (partialFulfillment)
              'partialRestoredAt': FieldValue.serverTimestamp(),
          ...
          transaction.update(orderReference, {
            ...
            if (delivered) 'completedAt': FieldValue.serverTimestamp(),
      ...
      createNotificationInTransaction(
        ...
        orderId: documentId,
        newStatus: newStatus,
        fulfilledQuantity: newStatus.toLowerCase() == 'accepted'
            ? fulfilledQuantity
            : null,
      );
    });
  ...
  Future<void> restoreUnfulfilledQuantity({
    ...
    if (stockId.isEmpty) {
      throw StateError('The stock record for this order is unavailable.');
    }
    ...
    if (!stockSnapshot.exists) {
      throw StateError('The stock record for this order no longer exists.');
    }

    final stockData = stockSnapshot.data() ?? <String, dynamic>{};

    final restoredStock = StockState.quantity(stockData) + quantityToRestore;
    ...
    transaction.update(stockReference, {
      ...
      'lastPartialFulfillmentOrderId': orderId,
      'updatedAt': FieldValue.serverTimestamp(),
    });
  }
  ...
  Future<bool> restoreStockIfNeeded({
    ...
    final fulfilledQuantity = OrderHelpers.getDoubleValue(
      orderData,
      'fulfilledQuantity',
    );
    final currentStatus = OrderHelpers.getStringValue(
    ...
    final quantityToRestore =
        currentStatus == 'accepted' && fulfilledQuantity > 0
        ? fulfilledQuantity
        : orderedQuantity;
